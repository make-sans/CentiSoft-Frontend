...
$("#getProjectsForm").submit(function (e) {
    ...
    let customerId = $(this).find("#customerIdInput").val();
    if (isNaN(customerId) || !customerId.length > 0) {
        alert("Given customer id is not a number!");
        return;
    }
    ...
    let url = hostname + ":" + port + "/api/customers/" + customerId + "/projects";
    ...
    $.ajax({
        ...
        .done(function (msg) {
            // clear last results
            let projectTableBody = document.querySelector("#projectsTable").getElementsByTagName("tbody")[0];
            ...
            if (!$.trim(msg)) {
                ...
                $.each(msg, function (key, value) {
                    ...
                    clone.querySelector(".project-id").innerHTML = value.Id;
                    clone.querySelector(".project-name").innerHTML = value.Name;
                    clone.querySelector(".project-duedate").innerHTML = value.DueDate;
                    clone.querySelector(".project-customerid").innerHTML = value.CustomerId;
                    clone.querySelector(".delete-this").value = value.CustomerId + "|" + value.Id;
...
$("#addNewProjectForm").submit(function (e) {
    ...
    // get the customer id input
    let customerId = $(this).find("#projectCustomerIdInput").val();
    if (isNaN(customerId) || !customerId.length > 0) {
...
$("#projectsTable").on("click", ".delete-this", function () {
    let deleteValue = $(this).val().split("|");
    let customerId = deleteValue[0];
    let deleteId = deleteValue[1];
    let url = hostname + ":" + port + "/api/customers/" + customerId + "/projects/" + deleteId;
    ...
    $.ajax({
        ...
        .done(function () {
            // clear last results
            $("#projectsTable th.project-id").filter(function () {
...
$("#getTasksForm").submit(function (e) {
    ...
    $.ajax({
        ...
        .done(function (msg) {
            // clear last results
            let taskTableBody = document.querySelector("#tasksTable").getElementsByTagName("tbody")[0];
            ...
            if (!$.trim(msg)) {
                ...
                $.each(msg, function (key, value) {
                    ...
                    clone.querySelector(".task-description").innerHTML = value.Description;
                    ...
                    clone.querySelector(".task-developerid").innerHTML = value.DeveloperId
                    clone.querySelector(".delete-this").value = value.ProjectId + "|" + value.Id;
...
$("#addNewTaskForm").submit(function (e) {
    ...
    // get the customer id input
    let projectId = $(this).find("#taskProjectIdInput").val();
    if (isNaN(projectId) || !projectId.length > 0) {
    ...
    $.ajax({
        ...
        url: url,
        headers: { "centisoft_token": "VerySecretToken1" },
        data: $(this).serialize() + "&Created=" + new Date().toISOString().slice(0, 19).replace('T', ' ')
    })
        .done(function () {
...
$("#tasksTable").on("click", ".delete-this", function () {
    ...
    $.ajax({
        ...
        .done(function () {
            // clear last results
            $("#tasksTable th.task-id").filter(function () {
                // Matches exact string   
                return $(this).text() === deleteId;
            }).parent().remove();
